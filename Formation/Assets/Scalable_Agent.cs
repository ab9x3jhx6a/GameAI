...
public class Scalable_Agent: MonoBehaviour {
	...
	void Start () {
		fix = transform.position - transform.parent.transform.position;
		fix.x -= 1.2f;
		fix.y -= -1.8f;
	...
	Vector3 Back(){
		Vector3 lookat = transform.parent.transform.position + fix;
		float angle = get_angle (lookat.x, lookat.y, x, y);
